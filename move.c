...
void turn_left(t_cub3d *t, double rot_spd, double mouse_spd)
{
    double old_p_dir_x;
    double old_plane_x;
    ...
    {
        old_p_dir_x = t->p_dir_x;
        t->p_dir_x = t->p_dir_x * cos(rot_spd * mouse_spd) - t->p_dir_y * sin(rot_spd * mouse_spd);
        t->p_dir_y = old_p_dir_x * sin(rot_spd * mouse_spd) + t->p_dir_y * cos(rot_spd * mouse_spd);
        old_plane_x = t->plane_x;
        t->plane_x = t->plane_x * cos(rot_spd * mouse_spd) - t->plane_y * sin(rot_spd * mouse_spd);
        t->plane_y = old_plane_x * sin(rot_spd * mouse_spd) + t->plane_y * cos(rot_spd * mouse_spd);

    }
}

void turn_right(t_cub3d *t, double rot_spd, double mouse_spd)
{
    double old_p_dir_x;
    double old_plane_x;
    ...
    if (t->mouse_move > 0)
    {
        ...
        old_plane_x = t->plane_x;
        t->plane_x = t->plane_x * cos(rot_spd * mouse_spd) - t->plane_y * sin(rot_spd * mouse_spd);
        t->plane_y = old_plane_x * sin(rot_spd * mouse_spd) + t->plane_y * cos(0.001 * mouse_spd);
    }
}
...
void            move(t_cub3d *t)
{
    ...
    if (t->l_a == 1 || t->mouse_move < 0)
        turn_left(t, rot_spd, mouse_spd);
    if (t->r_a == 1 || t->mouse_move > 0)
        turn_right(t, rot_spd, mouse_spd);
    if (t->a == 1)
