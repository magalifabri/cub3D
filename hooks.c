...
int mouse_move_hook(int current_mouse_x, int current_mouse_y, t_cub3d *t)
{
    ...
    if (current_mouse_x == prev_mouse_x)
        return (0);
    if (current_mouse_x == t->win_w / 2 && t->mouse_move == 0)
        prev_mouse_x = t->win_w / 2;
    if (current_mouse_x >= t->win_w && prev_mouse_x < t->win_w - 100)
        current_mouse_x = (t->win_w / 2) + t->mouse_move;
    else if (current_mouse_x <= 0 && prev_mouse_x > 100)
        current_mouse_x = (t->win_w / 2) + t->mouse_move;
    t->mouse_move = current_mouse_x - prev_mouse_x;
    prev_mouse_x = current_mouse_x;
    if (current_mouse_x <= 0)
    {
        mlx_mouse_move(t->win, t->win_w / 2, t->win_h / 2);
        prev_mouse_x = current_mouse_x + t->win_w / 2;
    }
